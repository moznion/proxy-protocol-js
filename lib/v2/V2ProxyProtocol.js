...
var V2ProxyProtocol = (function () {
    ...
    V2ProxyProtocol.prototype.build = function () {
        ...
        else if (this.addressFamily === AddressFamily_1.AddressFamily.INET6 && this.proxyAddress instanceof IPv6ProxyAddress_1.IPv6ProxyAddress) {
            for (var i = 0; i < 16; i++) {
                proto[cursor++] = this.proxyAddress.sourceAddress.address[i];
            }
            for (var i = 0; i < 16; i++) {
        ...
        else if (this.addressFamily === AddressFamily_1.AddressFamily.UNIX && this.proxyAddress instanceof UnixProxyAddress_1.UnixProxyAddress) {
            for (var i = 0; i < 108; i++) {
                proto[cursor++] = this.proxyAddress.sourceAddress.address[i];
            }
            for (var i = 0; i < 108; i++) {
    ...
    V2ProxyProtocol.parse = function (input) {
        ...
        var addressFamilyLength = AddressFamily_1.AddressFamily.getLength(addressFamily);
        ...
        return new V2ProxyProtocol(command, transportProtocol, AddressFamily_1.AddressFamily.getFactoryMethod(addressFamily)(input.slice(16)), input.slice(addressFamilyLength + 16));
    };
